...
public class EmergencyController {
    ...
    @Operation(
            summary = "Creates new emergency report",
            security = @SecurityRequirement(name = "bearerAuth"))
    @PostMapping("/new")
    public ResponseEntity<?> newEmergency(@Valid @RequestBody CreateEmergencyRequest emergencyRequest) {
        return ResponseEntity.ok(emergencyService.newEmergency(emergencyRequest));
    }

    @PreAuthorize("hasRole('dispositor')")
    @Operation(
            summary = "Currently logged dispositor approves request of given id",
            security = @SecurityRequirement(name = "bearerAuth"))
    @PostMapping("/approve/{id}")
    public void approveEmergency(@Valid @RequestBody ApproveEmergencyRequest request, @PathVariable Integer id) {
        emergencyService.approveEmergency(request, id);
    }

    @PreAuthorize("hasRole('dispositor')")
    @Operation(
            summary = "Get all unapproved emergencies",
            security = @SecurityRequirement(name = "bearerAuth"))
    ...
    @PreAuthorize("hasRole('dispositor')")
    @Operation(
            summary = "Get all approved emergencies",
            security = @SecurityRequirement(name = "bearerAuth"))
    @GetMapping("/approved")
